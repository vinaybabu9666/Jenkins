...
if($_POST['butSubmit'] == "Submit"){
	if($_POST['act'] == "edit" && $_POST['cat_id'] != ""){
		...
		$query = "update tbl_jobs set cat_name='".$_POST['txtCatName']."',content='".base64_encode($_POST['txtDesc'])."' where inc_cat_id='".$_POST['cat_id']."'";
		$retUrl = $_POST['retPage']."?pageId=".$_REQUEST['pageId'];
	}else{
		if(mysql_num_rows(mysql_query("select * from tbl_jobs where cat_name='".$_POST['txtCatName']."'"))>0){
			$stat = "EXISTED";
		}else{
			$query = "insert into tbl_jobs(cat_name,content,dt_created) values('".$_POST['txtCatName']."','".base64_encode($_POST['txtDesc'])."', current_date())";
...
?>	
					<tr>
    					<td width="22%" align="center" valign="middle" class="successMsg">Category has been updated successfully.</td>
					</tr>
<?php
	}
	if($stat == "EXISTED"){     
...
if(is_numeric($_REQUEST['cat_id']) && $_REQUEST['act']=="edit"){
	...
	$query = "select * from tbl_jobs where inc_cat_id=".$cat_id;
	$result = mysql_query($query);
...
?>
    		...
    		<table width="80%" border="0" align="center" cellpadding="0" cellspacing="6">
      			<tr>
        			<td width="22%" align="right" valign="middle" class="bold_txt"> New Job:</td>
...
//	}
	...
	?>				</td>
		...
		<td width="78%" align="left" valign="middle">
         <textarea name="txtDesc"  class="forTextfield" id="txtDesc" style="width:90%;height:165px;"><?=$content?></textarea>
